...
const projectItems = [
  ...
  },
  {
    ...
    role: [
      ...
      'Jenkins 기반 Backend·Frontend 빌드 Pipeline 구성',
      'Docker Image 빌드 및 Nexus Registry 연동',
      'Helm·Argo CD 기반 Kubernetes 자동 배포 구성',
      'Prometheus·Grafana 기반 모니터링 구성',
    ],
    details: [
      '애플리케이션 소스와 Kubernetes 배포 설정을 별도 GitHub Repository로 분리했습니다.',
      'Jenkins에서 Spring Boot Backend를 빌드·테스트하고 React Frontend와 함께 Docker Image로 생성해 Nexus에 저장했습니다.',
      'Git 커밋 ID를 Image Tag로 사용하고 Jenkins가 Helm values의 Tag를 변경하도록 구성했습니다.',
      'Argo CD가 Manifest 변경을 감지해 Kubernetes에 자동 배포하도록 연결했습니다.',
      'Helm으로 Frontend·Backend Replica, Service, Ingress와 Health Check 설정을 관리했습니다.',
      'Prometheus와 Grafana로 Kubernetes Cluster와 Application 상태를 확인할 수 있게 구성했습니다.',
    ],
    troubleshooting: [
      {
        problem: 'Jenkins에서 Nexus와 GitHub Repository에 접근하기 위한 인증정보를 안전하게 관리해야 했습니다.',
        solution: 'Nexus 계정은 Jenkins Credentials로 관리하고, GitHub 연결에는 빌드 시 발급되는 GitHub App Token을 사용했습니다.',
        result: 'Jenkinsfile에 비밀번호와 Token을 직접 저장하지 않고 Pipeline을 연결했습니다.',
      },
    ],
    improvements: [
      '이 프로젝트는 애플리케이션 빌드부터 Manifest 갱신과 Kubernetes 배포까지 이어지는 GitOps 전체 흐름을 검증하는 것이 목적이어서 Backend와 Frontend를 하나의 Jenkins Pipeline으로 구성했습니다. 실제 서비스 규모가 커지고 각 영역의 배포 주기가 달라질 경우 역할별 Pipeline으로 분리할 수 있도록 디렉터리와 Docker Image는 구분해 관리했습니다.',
    ],
    github: '',
    links: [
