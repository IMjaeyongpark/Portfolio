...
const projectItems = [
  {
    ...
    description: '사내 테스트 서버 VM 생성·설정을 코드로 자동화',
    period: '2026.08 — 2026.09',
    skills: ['Hyper-V', 'Rocky Linux', 'Terraform', 'Ansible', 'Packer', 'PowerShell', 'IaC'],
    purpose: '프로젝트마다 반복하던 WEB·WAS·DB 테스트 VM 생성을 코드로 관리하기 위해 Hyper-V 기반 IaC 환경을 만들었습니다. 현재 사내 테스트 서버에 도입해 사용 테스트 중입니다.',
    role: [
      'Packer·Kickstart로 Rocky Linux 기본 이미지 생성',
      'servers.yml·Terraform으로 VM 사양과 IP 관리',
      'Ansible로 OS·Docker·네트워크 설정 자동화',
      'PowerShell로 VM 생성·검증·삭제 과정 자동화',
    ],
    details: [
      'Packer와 Kickstart로 Rocky Linux 9.7 기본 이미지를 만들었습니다.',
      'servers.yml에 VM 이름·CPU·메모리·디스크·IP를 정의하고 Terraform으로 Hyper-V VM을 생성·관리합니다.',
      'VM 생성 후 Ansible 제어 VM에서 고정 IP·Docker·디스크·SSH·방화벽 설정을 적용합니다.',
      'PowerShell 검증 스크립트와 모의 테스트로 중복 VM·디스크 충돌, 재적용과 삭제 흐름을 확인했습니다.',
      '현재 사내 테스트 서버에 적용해 실제 사용 흐름을 테스트하고 있습니다.',
    ],
    challenges: [
      {
        problem: '신규 프로젝트를 수행할 때마다 WEB·WAS·DB 서버용 Hyper-V VM을 수동으로 반복 생성·설정해야 했습니다.',
        solution: '기본 이미지를 만든 뒤 서버 사양과 IP만 servers.yml에 입력하면 Terraform·PowerShell로 VM을 만들고 Ansible로 필요한 설정을 적용하도록 연결했습니다.',
