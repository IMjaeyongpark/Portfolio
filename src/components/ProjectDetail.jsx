      ...
      <article aria-labelledby="project-detail-title">
        ...
        <div className="page-container py-12 sm:py-16">
          ...
          <div className="rounded-2xl border border-line bg-white px-5 shadow-[0_18px_50px_rgba(15,23,42,0.04)] sm:px-8">
            ...
            {project.improvements?.length > 0 && <DetailSection number={project.troubleshooting?.length > 0 ? '05' : '04'} title="확장 방향"><ul className="space-y-3">{project.improvements.map((item) => <li key={item} className="flex gap-3"><span className="mt-2.5 text-[10px] text-lime">—</span>{item}</li>)}</ul></DetailSection>}
          </div>
