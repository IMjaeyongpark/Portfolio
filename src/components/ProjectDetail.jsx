...
export default function ProjectDetail({ project, backHref = '/#projects' }) {
  ...
  const caseSections = [
    ...
    { key: 'troubleshooting', title: '트러블슈팅', problemLabel: '발생한 문제', solutionLabel: '해결 방법', problemClass: 'text-red-500', items: project.troubleshooting },
              ...
              <DetailSection key={section.key} number={String(4 + sectionIndex).padStart(2, '0')} title={section.title}>
                <div className="space-y-6">
                  {section.items.map((item, index) => (
                    <div key={`${item.problem}-${index}`} className="space-y-4 rounded-2xl border border-line bg-[#f7f8fa] p-5">
                      <div><span className={`text-[10px] font-semibold tracking-wider ${section.problemClass}`}>{section.problemLabel}</span><p className="mt-1">{item.problem}</p></div>
                      <div><span className="text-[10px] font-semibold tracking-wider text-lime">{section.solutionLabel}</span><p className="mt-1">{item.solution}</p></div>
                      <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3"><span className="text-[10px] font-semibold tracking-wider text-blue-700">결과</span><p className="mt-1 font-medium text-[#333d4b]">{item.result}</p></div>
                    </div>
                  ))}
                </div>
              </DetailSection>
